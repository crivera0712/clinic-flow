...
function ScheduleCard({
  ...
  return (
    ...
    >
      <CardContent
        sx={{
          p: { xs: 2.5, lg: large ? 3.5 : 2.75 },
          pl: { xs: 3.25, lg: large ? 4.25 : 3.5 },
          "&:last-child": { pb: { xs: 2.5, lg: large ? 3.5 : 2.75 } },
        }}
      >
        <Stack direction="row" spacing={2} justifyContent="space-between" alignItems="flex-start">
          <Box sx={{ minWidth: 0 }}>
            <Typography
              sx={{
                color: accent,
                fontSize: large ? "clamp(1rem, 1.35vw, 1.4rem)" : "clamp(.9rem, 1vw, 1.1rem)",
                fontWeight: 750,
              }}
            >
              {row.therapistName}
            ...
            <Typography
              sx={{
                mt: 0.6,
                fontSize: large ? "clamp(1.8rem, 3vw, 3.35rem)" : "clamp(1.35rem, 2vw, 2.05rem)",
                fontWeight: 760,
                lineHeight: 1.08,
                letterSpacing: "-.03em",
              }}
            ...
            <Typography
              ...
              sx={{
                mt: 1,
                fontSize: large ? "clamp(.95rem, 1.25vw, 1.3rem)" : "clamp(.85rem, 1vw, 1.05rem)",
                fontWeight: 600,
              }}
            >
              {typeLabel(row.type)}
          ...
          <Chip
            ...
            sx={{
              ...
              fontSize: large ? "clamp(1rem, 1.45vw, 1.5rem)" : "clamp(.9rem, 1vw, 1.1rem)",
            }}
          />
        </Stack>
      </CardContent>
  ...
  return (
    <Box
      sx={{
        ...
        p: { xs: 2, sm: 3, lg: 4 },
        background:
          "radial-gradient(circle at 0% 0%, rgba(56,189,248,.08), transparent 28%), #050b18",
      }}
    >
      <Stack spacing={{ xs: 3, lg: 4 }} sx={{ maxWidth: 1800, mx: "auto" }}>
        ...
        >
          <Typography
            component="h1"
            sx={{ fontSize: "clamp(1.45rem, 2.3vw, 2.5rem)", fontWeight: 750, lineHeight: 1.15 }}
          >
            {CLINIC_NAME}
          </Typography>
          <Stack
          ...
          >
            ...
            <Box sx={{ textAlign: "right" }}>
              <Typography
                sx={{ fontSize: "clamp(1.35rem, 2vw, 2.2rem)", fontWeight: 720, lineHeight: 1 }}
              >
                {currentTime}
              </Typography>
              <Typography
                color="text.secondary"
                sx={{ mt: 0.75, fontSize: "clamp(.85rem, 1.1vw, 1.15rem)" }}
              >
                {currentDate}
              </Typography>
            </Box>
        ...
        <Stack spacing={1.5}>
          <Stack direction="row" spacing={1.25} alignItems="center">
            ...
            <Typography
              component="h2"
              sx={{ fontSize: "clamp(1.45rem, 2.1vw, 2.35rem)", fontWeight: 750 }}
            >
              Currently waiting
            </Typography>
          ...
          <Paper sx={{ p: { xs: 1.5, sm: 2, lg: 2.5 }, bgcolor: alpha(colors.surface, 0.82) }}>
            {waiting.length === 0 ? (
              <EmptyPanel
                ...
                description="Checked-in patients will appear here."
              />
            ) : (
              <Box
                sx={{
              ...
              >
                {waiting.map((row) => (
                  <ScheduleCard
                    ...
                    row={row}
                    prominence="primary"
                    accent={therapistAccents.get(row.therapistId)!}
                  />
                ))}
              </Box>
            )}
        ...
        <Stack spacing={1.5}>
          <Stack direction="row" spacing={1.25} alignItems="center">
            ...
            <Typography
              component="h2"
              sx={{ fontSize: "clamp(1.2rem, 1.55vw, 1.65rem)", fontWeight: 720 }}
            >
              Up next
            </Typography>
          ...
          <Paper sx={{ p: { xs: 1.5, sm: 2 }, bgcolor: alpha(colors.surface, 0.5) }}>
            {upNext.length === 0 ? (
              <EmptyPanel
                ...
                description="Scheduled patients will appear here."
              />
            ) : (
              <Box
                sx={{
