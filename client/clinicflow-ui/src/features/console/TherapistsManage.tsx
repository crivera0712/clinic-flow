...
export function TherapistsManage() {
  ...
  return (
    <Stack spacing={3.5}>
      <PageHeader eyebrow="Care team" title="Therapists" description="Manage the clinicians available for scheduling and daily patient care." />
      ...
      <Paper sx={{ p: { xs: 2, md: 2.5 }, bgcolor: colors.surface }}>
        <Stack direction="row" spacing={1.25} alignItems="center" sx={{ mb: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">New therapists are immediately available in scheduling.</Typography>
          </Box>
        </Stack>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} component="form" onSubmit={(e) => { e.preventDefault(); void handleAdd(); }}>
          <TextField label="Therapist name" value={name} onChange={(e) => setName(e.target.value)} required sx={{ flex: 1, ...adminTextFieldSx }} />
          <Button type="submit" variant="contained" startIcon={<AddRoundedIcon />} disabled={adding}>Add therapist</Button>
        </Stack>
      </Paper>
      ...
      <Paper sx={{ overflow: "hidden", bgcolor: colors.surface }}>
        <Box sx={{ px: { xs: 2, md: 2.5 }, py: 2, borderBottom: `1px solid ${colors.borderSoft}` }}>
          ...
          <Typography variant="body2" color="text.secondary">{loading ? "Loading care team…" : `${therapists.length} therapist${therapists.length === 1 ? "" : "s"}`}</Typography>
        </Box>
        {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
        {!loading && therapists.length === 0 ? (
          <EmptyState icon={<MedicalServicesRoundedIcon sx={{ fontSize: 42 }} />} title="No therapists yet" description="Add the first member of your care team above." />
        ) : (
        <Stack divider={<Box sx={{ borderBottom: `1px solid ${colors.borderSoft}` }} />}>
          {therapists.map((t) => (
            <Stack key={t.id} direction="row" spacing={2} alignItems="center" sx={{ px: { xs: 2, md: 2.5 }, py: 1.75, "&:hover": { bgcolor: alpha(colors.surfaceRaised, 0.45) } }}>
              <Avatar sx={{ bgcolor: alpha(colors.success, 0.13), color: "success.main", fontWeight: 750 }}>{t.name.charAt(0).toUpperCase()}</Avatar>
              <Box sx={{ flexGrow: 1 }}>
                <Typography fontWeight={700}>{t.name}</Typography>
                <Typography variant="body2" color="text.secondary">Available for appointments</Typography>
              </Box>
              <Tooltip title={`Remove ${t.name}`}>
                <IconButton aria-label={`Remove ${t.name}`} color="error" onClick={() => setDeleteTarget(t)}><DeleteOutlineRoundedIcon /></IconButton>
              </Tooltip>
            </Stack>
          ))}
        </Stack>
